...
public class ClientesDAOImpl implements IClientesDAO
{
	...
	public void excluir(String cpf) throws GlobalcodeException
	{
		...
		{
			throw new GlobalcodeException("Erro ao excluir cliente: " + DELETE_CLIENTE, e);
		}
		finally
		{
			ConnectionManager.closeAll(conn, stmt);
	...
	public void salvar(Cliente cliente) throws GlobalcodeException
	{
		...
		{
			throw new GlobalcodeException("Erro ao salvar cliente: " + SALVAR_CLIENTE, e);
		}
		finally
		{
	...
	{
		/*
		 * Consultar um cliente no banco de dados usando a constante GET_CLIENTE_BY_CPF.
		 */
		...
		// Declarar um objeto tipo Cliente
		Cliente clie = null;
		...
		try
		{
			...
			while (rs.next())
			{
				...
				String nome = rs.getString("nome");
				String telefone = rs.getString("telefone");
				clie = new Cliente(nome, telefone, cpf, id);
			}
		}
		catch (SQLException e)
		{
			throw new GlobalcodeException("Nao foi possivel recuperar o cliente: " + GET_CLIENTE_BY_CPF, e);
		}
		finally
		{
			ConnectionManager.closeAll(conn, stmt, rs);
