...
public class ClienteDAOSerialization implements ClienteDAO
{
	...
	public List<Cliente> getAllClientes() throws GlobalcodeException
	{
		 ...
		 *
		 */
		String filtro = "Cliente*.ser";

		try (DirectoryStream<Path> arquivosFiltrados = Files.newDirectoryStream(diretorioAtual, filtro))
		{
			...
			for (Path caminho : arquivosFiltrados)
			{
				...
				/*
				 * Observacao: Como a palavra cliente tem 7 caracteres e a extensao (.ser) ocupa
				 * 4 caracteres podemos utilizar o seguinte comando para obtermos somente o cpf,
				 * eliminando o inicio (Cliente) e o fim (.ser) String cpf =
				 * umArquivo.substring(7,umArquivo.length() -4 );
				 */
	...
	public Cliente getClienteByCPF(String cpf) throws GlobalcodeException
	{
		...
		Cliente c = null;
		try (FileInputStream fis = new FileInputStream(nomeArquivo); ObjectInputStream ois = new ObjectInputStream(fis))
		{
